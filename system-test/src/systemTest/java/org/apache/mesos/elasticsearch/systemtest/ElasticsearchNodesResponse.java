    ...
    class ElasticsearchNodesCall implements Callable<Boolean> {
        ...
        private boolean endpointIsOk(JSONObject task) {
            ...
            if (200 <= response.getStatus() && response.getStatus() < 400) {
                ...
                if (body.getObject().getJSONObject("nodes").length() != 3) {
                    DiscoverySystemTest.LOGGER.info("Polling Elasticsearch endpoint '" + url + "' returned wrong number of nodes: " + body.getObject().getJSONObject("nodes").length());
                    return false;
                }
                else {
